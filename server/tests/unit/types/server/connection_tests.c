...
Test(server_restrict_controller_tests, restrict_disconnected_controller)
{
    // Arrange
    server_t *server = server_new();
    controller_t *controller = controller_new(42);
    ...
    // Pre-assert
    cr_assert_eq(controller->generic.state, CTRL_CONNECTED);
    fd_states_set(&server->fd_actual, 42, FD_STATES_R | FD_STATES_W | FD_STATES_E);
    fd_states_set(&server->fd_watch, 42, FD_STATES_R | FD_STATES_W | FD_STATES_E);

    // Act
    server_restrict_controller(server, controller, CTRL_DISCONNECTED);

    // Assert
    cr_assert_eq(controller->generic.state, CTRL_DISCONNECTED);
    cr_assert_not(FD_ISSET(42, &server->fd_watch.readable));
    cr_assert_not(FD_ISSET(42, &server->fd_watch.writable));
    cr_assert(FD_ISSET(42, &server->fd_watch.except));
    cr_assert_not(FD_ISSET(42, &server->fd_actual.readable));
    cr_assert_not(FD_ISSET(42, &server->fd_actual.writable));
    cr_assert(FD_ISSET(42, &server->fd_actual.except));
...
Test(server_restrict_controller_tests, restrict_requests_controller)
{
    // Arrange
    server_t *server = server_new();
    controller_t *controller = controller_new(42);
    ...
    // Act
    server_restrict_controller(server, controller, CTRL_ALLOW_EMIT);

    // Assert
    cr_assert(CTRL_CAN_EMIT(controller));
    cr_assert_not(CTRL_CAN_REQ(controller));
    cr_assert_not(FD_ISSET(42, &server->fd_watch.readable));
    cr_assert(FD_ISSET(42, &server->fd_watch.writable));
    cr_assert(FD_ISSET(42, &server->fd_watch.except));
    cr_assert_not(FD_ISSET(42, &server->fd_actual.readable));
    cr_assert(FD_ISSET(42, &server->fd_actual.writable));
    cr_assert(FD_ISSET(42, &server->fd_actual.except));
...
Test(server_restrict_controller_tests, restrict_emissions_controller)
{
    // Arrange
    server_t *server = server_new();
    controller_t *controller = controller_new(42);
    ...
    // Act
    server_restrict_controller(server, controller, CTRL_ALLOW_REQ);

    // Assert
    cr_assert_not(CTRL_CAN_EMIT(controller));
    cr_assert(CTRL_CAN_REQ(controller));
    cr_assert(FD_ISSET(42, &server->fd_watch.readable));
    cr_assert_not(FD_ISSET(42, &server->fd_watch.writable));
    cr_assert(FD_ISSET(42, &server->fd_watch.except));
    cr_assert(FD_ISSET(42, &server->fd_actual.readable));
    cr_assert_not(FD_ISSET(42, &server->fd_actual.writable));
    cr_assert(FD_ISSET(42, &server->fd_actual.except));
...
Test(server_restrict_controller_tests, restrict_null_controller)
{
    // Arrange
    server_t *server = server_new();

    // Act
    server_restrict_controller(server, NULL, CTRL_ALLOW_REQ);
