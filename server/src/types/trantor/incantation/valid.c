...
static bool incantation_check_resources(incantation_t *incantation,
    const size_t resources[RES_LEN], map_t *map)
{
    map_cell_t *cell = MAP_CELL_AT_POS(map, incantation->location);
...
bool incantation_is_valid(incantation_t *incantation, map_t *map)
{
    ...
    if (incantation->players->len < requirement->nb_players)
        return false;
    return incantation_check_resources(incantation,
