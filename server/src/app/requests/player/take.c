...
bool app_handle_player_request_take_onstart(
    ...
    request_t *request)
{
    ...
    if (request_get_token(request, 2, &token)) {
        log_warn("Too many arguments for 'Take' command");
        return false;
    }
    if (!request_get_token(request, 1, &token)) {
        log_warn("Not enough arguments for 'Take' command");
        return false;
    }
    ...
    __attribute_maybe_unused__ request_t *request)
{
    ...
    if (!player_take_object(controller->player, app->world->map, resource)) {
        controller_add_emission((controller_t *) controller, "ko\n");
        return;
    }
